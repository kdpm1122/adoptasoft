...
module.exports = async (req, res) => {
  ...
  const result = await pool.query('DELETE FROM mascotas WHERE id_mascota = $1', [id]);
  if (result.rowCount === 0) return jsonError(res, 'Mascota no encontrada.', 404);

  jsonResponse(res, { message: 'Mascota eliminada.' });
};
